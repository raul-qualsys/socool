...
function fn_update_product_option($option_data, $option_id = 0, $lang_code = DESCR_SL)
{
    ...
    if (empty($option_id)) {
        $action = 'create';
        if (empty($option_data['product_id']) || !empty($option_data['linked_product_id'])) {
        ...
        $create = true;
        if (!empty($option_data['linked_product_id'])) {
            fn_add_global_option_link($option_data['linked_product_id'], $option_data['option_id']);
        }
    // Update option
    } else {
        $action = 'update';
